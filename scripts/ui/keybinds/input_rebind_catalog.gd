...
static func entries() -> Array[Dictionary]:
	return [
		...
		_entry("fly_ascend", "Ascend", GROUP_FLYING),
		_entry("fly_descend", "Descend", GROUP_FLYING),
...
static func is_allowed_bind_overlap(action_a: String, action_b: String) -> bool:
	if action_a == action_b:
		return true
	if _is_flight_vs_ground_loco(action_a, action_b):
		return true
...
static func _is_flight_vs_ground_loco(action_a: String, action_b: String) -> bool:
	var a_flight := group_of(action_a) == GROUP_FLYING
	var b_flight := group_of(action_b) == GROUP_FLYING
	if a_flight == b_flight:
		return false
	var other := action_b if a_flight else action_a
	return other == "jump" or other == "crouch" or other == "dash"
