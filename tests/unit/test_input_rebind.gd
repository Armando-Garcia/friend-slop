...
func _catalog_error(names: PackedStringArray) -> String:
	...
	if err.is_empty():
		err = _catalog_flying_section_error()
...
func _catalog_flying_section_error() -> String:
	var saw_movement := false
	var saw_flying := false
	for entry in CatalogScript.entries():
		var group := str(entry.get("group", ""))
		if group == CatalogScript.GROUP_MOVEMENT:
			saw_movement = true
		elif group == CatalogScript.GROUP_FLYING:
			if not saw_movement:
				return "Flying must come after Movement"
			saw_flying = true
		elif saw_movement and not saw_flying:
			return "Flying must be the section directly below Movement"
		var action := str(entry.get("action", ""))
		if action == "fly_ascend" or action == "fly_descend":
			if group != CatalogScript.GROUP_FLYING:
				return "%s must be in Flying" % action
	if not saw_flying:
		return "Catalog must include a Flying section"
	return ""
...
func _test_source_guards() -> int:
	...
	var broom := FileAccess.get_file_as_string("res://scripts/headmaster/broom_flight.gd")
	...
	if player.find('is_action_pressed("spell_capture")') < 0:
		...
		return 1
	if broom.find('is_action_pressed("dash")') < 0:
		push_error("broom_flight.gd must boost on dash")
		return 1
	if broom.find('is_action_pressed("fly_ascend")') < 0:
		push_error("broom_flight.gd must ascend on fly_ascend")
		return 1
